...
{
  ofstream fout;
  fout.open("src/model/models/skySphere.d");
  if (fout.fail()) return 1;

  int latitude   = 30; // Left and right
  int longitude  = 30; // Up and Down
  int numVertices = 2 + ((longitude - 2) * latitude);
  fout << fixed << numVertices << endl;

  // Top Vertex
  fout << 0 << " " << 1 << " " << 0 << " 1    0 0 1 0\n";
